...
console.log(
  'This script populates some test books, authors, genres and bookinstances to your database. Specified database as argument - e.g.: node populatedb "mongodb+srv://hansiganter:hykdut6syvBifivqap@cluster0.bzb0bdz.mongodb.net/local_library?retryWrites=true&w=majority"'
);
...
async function main() {
  ...
  console.log("---First of all, empty database");
  await emptyDatabase();
...
async function emptyDatabase() {
  ...
  db.listCollections().toArray((err, collections) => {
    ...
    collections.forEach((collection) => {
      ...
      db.collection(collection.name).deleteMany({}, (err, result) => {
        if (err) {
          console.error(`Error emptying collection ${collection.name}:`, err);
        } else {
          console.log(`Emptied collection ${collection.name}`);
        }
      });
    });
  });
